...
- **Só o detector pode gerar `SAFETY_EVENTS`/`SYNC_EVENTS`**: o backend rastreia a **conexão WebSocket** que ganhou o `DETECTOR_CLAIM` (`ConnectionManager._detector_connection`) e rejeita esses eventos de qualquer outra conexão — inclusive se o `session_id` no corpo da mensagem for igual ao do detector real. Isso existe porque `session_id` é um valor **autodeclarado pelo próprio cliente** a cada mensagem, e não deveria (e antes desta correção não fazia) ser usado como prova de identidade: sem essa checagem, um viewer (ou qualquer device com o mesmo token compartilhado) podia mandar `ALARM_ACKNOWLEDGED` e silenciar o alarme real de outro dispositivo, ou forjar `CALIBRATION_SYNC`/`MODEL_SYNC` e sobrescrever o estado persistido para todo mundo. `DETECTOR_CLAIM` e `DETECTOR_RELEASE` seguem a mesma regra.
- **Limite de tamanho por mensagem**: mensagens WS acima de 512KB (`MAX_MESSAGE_BYTES`) são descartadas antes mesmo do parse — protege contra payloads gigantes/malformados sendo persistidos repetidamente em `safenap_shared.json`.
